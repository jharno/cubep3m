...
!!   -DCROSS: Output cross correlators as well as auto correlators (power).
...
program cic_power 
  ...
  integer num_checkpoints, cur_checkpoint, checkpoint_1, checkpoint_2
  ...
  !! Common block
#ifdef PLPLOT
 common xvp,send_buf,den_buf,den,recv_buf,pkdm,pkplot,slab2
#else
  common xvp,send_buf,den_buf,den,recv_buf,pkdm,poisson,slab2
#endif
...
#endif
  do checkpoint_1 = 1, num_checkpoints 
    cur_checkpoint = checkpoint_1
    power_flag = 0
    call initvar
    call read_particles
    call pass_particles
    call darkmatter
#ifdef CROSS
! save slab into a new array so that it is not overwritten
    slab2 = slab
    !save slab into a new array so that it is not overwritten
!    slab2 = slab
!    call PoissonNoise
!    if (rank == 0) call writepowerspectra
    do checkpoint_2 = checkpoint_1 + 1, num_checkpoints
       cur_checkpoint = checkpoint_2
       power_flag = 1
       call initvar
       call read_particles
       call pass_particles
...
!!------------------------------------------------------------------!!
  ...
  subroutine writepowerspectra
    ...
    write(z_write,'(f7.3)') z_checkpoint(checkpoint_1)
    z_write=adjustl(z_write)
#ifdef CROSS
    write(z_write2,'(f7.3)') z_checkpoint(checkpoint_2)
    z_write2=adjustl(z_write2)
#endif
...
#ifdef KAISER
# ifdef CROSS
    if (z_write .eq. z_write2) then
       fn=output_path//z_write(1:len_trim(z_write))//prefix//'-RSD.dat'
    else
       fn=output_path//z_write(1:len_trim(z_write))//'x'//z_write2(1:len_trim(z_write2))//prefix//'-RSD.dat' 
    endif
# else
    fn=output_path//z_write(1:len_trim(z_write))//prefix//'-RSD.dat'
# endif
...
# ifdef CROSS
    if (z_write .eq. z_write2) then
       ...
       fn=output_path//z_write(1:len_trim(z_write))//'x'//z_write2(1:len_trim(z_write2))//prefix//'.dat' 
    endif
# else
    fn=output_path//z_write(1:len_trim(z_write))//prefix//'.dat'
# endif
...
#ifdef CROSS
    if (power_flag==1) then
       call powerspectrum(slab,slab2,poisson)
    endif
#else
    call powerspectrum(slab,slab,poisson)
#endif
        ...
        do j = 1, nc_node_dim
            do i = 1, nc, 2
                ...
                if (kr .ne. 0) then
                    ...
                    w2=0
#endif                
                    pow=sum((delta(i:i+1,j,k)*delta2(i:i+1,j,k))/ &
                         real(ncr)**6)/kernel**4
