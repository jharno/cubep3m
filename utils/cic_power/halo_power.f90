...
! frequently changed parameters are found in this header file:
  ...
  character(len=200) :: halopath
!   character(len=200) :: outputfile
  character(len=7) :: z_string

  logical, parameter :: correct_kernel=.false.

  !character(len=*), parameter :: checkpoints='NOTUSED' !cubepm_root//'/input/checkpoints'
  ...
  !character(len=*), parameter :: checkpoints=cubepm_root//'/input/checkpoints_UBC_CLOSE'
  ...
  integer, parameter :: np=hc!2.5*ceiling(50000**(1.0/3.0))
...
!!---start main--------------------------------------------------------------!!
  ...
  !call getarg(1, halopath)
  !call getarg(2, z_string)
...
! ------ halo power stuff ----------------
    ...
    ! number of halo in header, which is MORE THAN the number of halo the catalog contains
    integer(4) max_nh
        
    ! structure of the halo catalog
    !real, dimension(6, max_np) :: xv (already declared above)
    !real(4), dimension(3, max_np) :: peakxv
    !real(4), dimension(3, max_np) :: angularMomentum 
    !real(4), dimension(max_np)    :: sigma_v, radius_scale, halo_mass1, halo_mass2, halo_mass3
    !real(4), dimension(3, max_np) :: var_shape

    real(4), dimension(22)        :: reading_buffer
...
!     check_name=halopath(1:len_trim(halopath))//z_string(1:len_trim(z_string))//'xv'// &
!                rank_string(1:len_trim(rank_string))//'.dat_rand'
    check_name=halopath(1:len_trim(halopath))//z_string(1:len_trim(z_string))//'halo'// &
               rank_string(1:len_trim(rank_string))//'.dat'
               
    write(*,*) 'Opening FILENAME = ', check_name(1:len_trim(check_name))
...
! #endif
    ...
    if (max_nh > max_np) then
      write(*,*) 'too many particles to store'
      write(*,*) 'rank',rank,'max_nh',max_nh,'max_np',max_np
      call mpi_abort(mpi_comm_world,ierr,ierr)
    endif
    
! count number of halos contained in catalog

    np_local = 0
    do j=1,max_np
        !read(21, end=111) peakxv(:,j)
        !read(21) xvp(:,j)
        !read(21) angularMomentum(:,j)
        !read(21) sigma_v(j)
        !read(21) radius_scale(j)
        !read(21) halo_mass1(j)
        !read(21) halo_mass2(j)
        !read(21) halo_mass3(j)
        !read(21) var_shape(:,j)
        read(21, end=111) reading_buffer(:)
        xvp(:,j)=reading_buffer(4:9)        
...
!         endif
    ...
    111 write(*,*) 'rank = ', rank, '  np_local = ', np_local, '  max_nh = ', max_nh
...
!! tally up total number of particles
    ...
    call mpi_bcast(np_total, 1, MPI_REAL, 0, MPI_COMM_WORLD, ierr)
...
!    fn=outputfile(1:len_trim(outputfile))
! #endif
...
!#endif
    ...
    fn = halopath(1:len_trim(halopath))//'/'//z_string(1:len_trim(z_string))//'halops.dat'
...
!     real, dimension(nc_node_dim, nc_node_dim) :: slice
!     character(len=100) :: slicename
    character(len=4) :: rank_string
...
!            slice(i,j) = 0
!            do k=1,nc_node_dim
!               slice(i,j) = slice(i,j) + cube(i,j,k)
!            enddo
!        enddo
!     enddo
!     
!     write(rank_string,'(i4)') rank
!     rank_string=adjustl(rank_string)
!     slicename = halopath(1:len_trim(halopath))//'/halo_slice'//rank_string(1:len_trim(rank_string))//'.dat'
!     
!     open(55, file=slicename, form='binary');
!     write(55) slice
!     close(55)
