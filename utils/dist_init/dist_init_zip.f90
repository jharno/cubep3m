!! write zip version of xv xv_zip[1-3].dat !!
...
!  character(*), parameter :: fntf='camb_jddefault_transfer_z0.dat'
  ...
  integer(4), parameter :: nc_node_dim = nc/nodes_dim, ncc=nc_node_dim/4
...
#endif
character(200) :: fzip1,fzip2,fzip3
integer(4) :: ii,jj,kk,pp, fstat, mesh_scale=4, v_resolution=16384
integer(4) :: ll(np_node_dim**3 * 1.5)
integer(4) :: hoc(ncc,ncc,ncc)
real(4) :: vmax_local, vmax, v_r2i, rhoc_i4
...
!! Open output file
 ...
 !! xv file
    ...
    call cpu_time(time2)
    time2=(time2-time1)
    if (rank == 0) write(*,"(f8.2,a)") time2,'  Wrote xvp'

!! construct a linked list
hoc=0
pp=0
print*,'dist_init_zip:dm: thread=',thread
pause
...
do ii=1,np_node_dim
  pp=pp+1
  i=floor(xvp(1,ii,jj,kk)/mesh_scale)+1
  j=floor(xvp(2,ii,jj,kk)/mesh_scale)+1
  k=floor(xvp(3,ii,jj,kk)/mesh_scale)+1
  if (i<1 .or. i>ncc .or. j<1 .or. j>ncc .or. k<1 .or. k>ncc) then
    print*,'dist_init_zip:dm: particle out of physical volume'
    call mpi_abort(mpi_comm_world,ierr,ierr)
  endif
  ll(pp)=hoc(i,j,k)
  hoc(i,j,k)=pp
...
!! fzip
#ifndef NEUTRINOS
  fzip1=output_path//'/node'//rank_s(1:len_trim(rank_s))//'/'//z_s(1:len_trim(z_s))//'zip1_'//rank_s(1:len_trim(rank_s))//'.dat'
  fzip2=output_path//'/node'//rank_s(1:len_trim(rank_s))//'/'//z_s(1:len_trim(z_s))//'zip2_'//rank_s(1:len_trim(rank_s))//'.dat'
  fzip3=output_path//'/node'//rank_s(1:len_trim(rank_s))//'/'//z_s(1:len_trim(z_s))//'zip3_'//rank_s(1:len_trim(rank_s))//'.dat'
#else
  fzip1=output_path//'/node'//rank_s(1:len_trim(rank_s))//'/'//z_s(1:len_trim(z_s))//'zip1_'//rank_s(1:len_trim(rank_s))//'_nu.dat'
  fzip2=output_path//'/node'//rank_s(1:len_trim(rank_s))//'/'//z_s(1:len_trim(z_s))//'zip2_'//rank_s(1:len_trim(rank_s))//'_nu.dat'
  fzip3=output_path//'/node'//rank_s(1:len_trim(rank_s))//'/'//z_s(1:len_trim(z_s))//'zip3_'//rank_s(1:len_trim(rank_s))//'_nu.dat'
#endif

open(unit=12, file=fzip1, status="replace", iostat=fstat, access="stream")
open(unit=13, file=fzip2, status="replace", iostat=fstat, access="stream")
open(unit=14, file=fzip3, status="replace", iostat=fstat, access="stream")

vmax_local = maxval(abs(xvp(4:6,:,:,:)))
call mpi_allreduce(vmax_local, vmax, 1, mpi_real, mpi_max, mpi_comm_world, ierr)
v_r2i = v_resolution/vmax
write(12) header_garbage, v_r2i
...
do i=1,ncc
  ...
  pp=hoc(i,j,k)
  do while (pp>0)
    kk = (pp-1)/np_node_dim**2 + 1
    jj = (pp - (kk-1)*np_node_dim**2 - 1)/np_node_dim + 1
    ii = pp - (kk-1)*np_node_dim**2 - (jj-1)*np_node_dim
    rhoc_i4=rhoc_i4+1
    write(11) int( fraction( xvp(1:3,jj,jj,kk)/mesh_scale ) * 256 ,kind=1)
    write(11) int( xvp(4:6,ii,jj,kk) * v_r2i ,kind=2)
    pp=ll(pp)
  enddo
  if (rhoc_i4>255) then
    write(12) int(rhoc_i4,kind=1)
  else
    write(12) int(255,1)
    write(13) rhoc_i4
  endif
