...
!! Modified by Joachim Harnois-Deraps to solve ODE with 
!! Convolution instead of finite difference
!!-----------------------------------------------------!!
...
!  character(*), parameter :: fntf='CAMB_BAO_Tf_CAMB_BAO.dat'
  ...
  integer, parameter :: new_nc=nc/2
  integer, parameter :: hc=new_nc/2
  real, parameter    :: ncr=new_nc
  real, parameter    :: hcr=hc

  !! np is the number of particles
  !! np should be set to nc (1:1), hc (1:2), or qc (1:4)
  integer, parameter :: np=new_nc!hc
  real, parameter    :: npr=np

  !! internal parallelization parameters
  integer(4), parameter :: nc_node_dim = new_nc/nodes_dim
  ...
  integer(4), parameter :: nc_slab = new_nc / nodes
...
!  real, dimension(7,nk) :: tf    !CAMB
  real, dimension(2,new_nc) :: pkm,pkn

  !! Fourier transform arrays
  real, dimension(nc_node_dim,nc_node_dim,nc_node_dim) :: cube!, new_phi
  real, dimension(nc_node_dim,nc_node_dim,nc_slab,0:nodes_slab-1) :: recv_cube
  real, dimension(new_nc+2,new_nc,nc_slab) :: slab, slab_work, phi_k
  real, dimension(0:nc_node_dim+1,0:nc_node_dim+1,0:nc_node_dim+1) :: phi
  real, dimension(0:nc_node_dim+1,0:nc_node_dim+1) :: phi_buf
  real, dimension(nc_node_dim,nc_node_dim,nc_node_dim, 3) :: dis_field
  ...
  !! Common block
  common /rvar/ tf,pkm,pkn,slab,phi!, phi_k, dis_field!,phi_buf
...
!! set up global mpi communicator
    ...
    if (mod(new_nc,nodes) /= 0) then
      write(*,*) 'cannot evenly decompose mesh into slabs'
      write(*,*) 'nc=',new_nc,'nodes=',nodes,'mod(nc,nodes) != 0'
      call mpi_abort(mpi_comm_world,ierr,ierr)
    endif
...
! initialize plan variables for fftw
    ...
    if (firstfftw) then
      call rfftw3d_f77_mpi_create_plan(plan,mpi_comm_world,new_nc, &
            new_nc,new_nc, FFTW_REAL_TO_COMPLEX, FFTW_ESTIMATE) !_MEASURE)
      call rfftw3d_f77_mpi_create_plan(iplan,mpi_comm_world,new_nc, &
            new_nc,new_nc, FFTW_COMPLEX_TO_REAL, FFTW_ESTIMATE) !_MEASURE)
#ifdef DEBUG_LOW
...
!! call pack routine if we are going forward
    ...
    if (command > 0) then
      ...
      slab=slab/(real(new_nc)*real(new_nc)*real(new_nc))
    endif
...
!!------------------------------------------------------------------!!
  ...
  subroutine dm
    implicit none
    integer :: i,j,k,ioerr,i3, ii,im 
    integer :: i1,j1,k1,lb,ub,kx,ky,kz,kg
    real    :: d,dmin,dmax,sum_dm,sum_dm_local,dmint,dmaxt,vf,xvp(6),kern,kr
    real*8  :: dsum,dvar,dsumt,dvart
...
!    !$omp end parallel do
    ...
    ! 2-Multiply phi_k by a force kernel in x,y,z, and fftw(-1)

    do i3=1,3

       slab=0
       
       !$omp parallel do default(shared) private(i,j,k,kg,kx,ky,kz,kr) &
       !$omp& private(im,ii,kern)
       do k=1,nc_slab
          kg=k+nc_slab*rank
          if (kg .lt. hc+2) then
             kz=kg-1
          ...
          do j=1,new_nc
             ...
             do i=1,new_nc+1,2
                kx=(i-1)/2
                kr=sqrt(real(kx)**2 + real(ky)**2 + real(kz)**2)
                if (kr .eq. 0) then
                   slab(i:i+1,j,k)=0.
                   !do I need this!?!?!?!?!?
                else
                   ...
                   im=i    !real
                   ii=i+1 !imaginary
                   if (i3 == 1) kern = -(2.*pi/ncr)*kx/4./pi
                   if (i3 == 2) kern = -(2.*pi/ncr)*ky/4./pi
                   if (i3 == 3) kern = -(2.*pi/ncr)*kz/4./pi

                   !write(*,*) i3,i,j,k,kx,ky,kz,kern 
                   

                   slab(im,j,k)=-phi_k(ii,j,k)*kern ! kern = k/4/pi
                   slab(ii,j,k)=phi_k(im,j,k)*kern

                   !write(*,*) phi_k(im,j,k), phi(ii,j,k), slab(im,j,k), slab(ii,j,k)
                   !pause
    ...
    do k=1,np_node_dim
       ...
       do j=1,np_node_dim
          ...
          do i=1,np_node_dim
             ...
             !write(*,*) k,k1,j,j1,i,i1
             !write(*,*) (dis_field(i1,j1,k1,1))
             !write(*,*) (dis_field(i1,j1,k1,2))
             !write(*,*) (dis_field(i1,j1,k1,3))
             !pause

             dis(1)=(dis_field(i1,j1,k1,1))
             dis(2)=(dis_field(i1,j1,k1,2))
             dis(3)=(dis_field(i1,j1,k1,3))
...
!!--------------------------------------------------------------!!
  ...
  subroutine powerspectrum(pk)
    implicit none
    real, dimension(2,new_nc)       :: pk

    integer :: i,j,k,kg
    integer :: k1,k2
    real    :: kr,kx,ky,kz,w1,w2,pow
    real, dimension(3,new_nc,nc_slab) :: pkt
    real, dimension(3,new_nc) :: pktsum
    ...
    !! Reduce to rank 0
    call mpi_reduce(pkt(:,:,1),pktsum,3*new_nc,mpi_real,mpi_sum,0,mpi_comm_world,ierr)
    ...
    call mpi_bcast(pk,2*new_nc,mpi_real,0,mpi_comm_world,ierr)
