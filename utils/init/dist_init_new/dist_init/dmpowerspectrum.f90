  subroutine dmpowerspectrum
    ...
    integer, parameter :: kpt=nc_slab/nt !nc_node_dim/nt
    integer, parameter :: npt=np_node_dim/nt

    integer it
    integer i,j,k
    integer k1,k2
    real kr,kx,ky,kz,w1,w2,kgz
    real p1,p2,p3,p4, sum_dm, sum_dm_local
    real*8 pst(5,nc,nt)
    ...
    sum_dm_local=sum(phi)
    do it=0,nodes-1
      call mpi_barrier(mpi_comm_world,ierr)
      if (rank == it) then
        write(*,*) 'rank',rank,'DM no placement=',sum_dm_local
      endif
    enddo
    call mpi_reduce(sum_dm_local,sum_dm,1,mpi_real,mpi_sum,0,mpi_comm_world,ierr)
    if (rank == 0) write(*,*) 'DM no placement global=',sum_dm 
    ...
    do it=0,nodes-1
      call mpi_barrier(mpi_comm_world,ierr)
      if (rank == it) then
        write(*,*) 'rank',rank,'DM with buffer sum=',sum_dm_local
      endif
    enddo
    call mpi_reduce(sum_dm_local,sum_dm,1,mpi_real,mpi_sum,0,mpi_comm_world,ierr)
    if (rank == 0) write(*,*) 'DM Sum with buffer Global=',sum_dm 
    ...
    do it=0,nodes-1
      call mpi_barrier(mpi_comm_world,ierr) 
      if (rank == it) then
        write (*,*) 'rank',rank,'DM Sum=',sum_dm_local
      endif
    enddo
    call mpi_reduce(sum_dm_local,sum_dm,1,mpi_real,mpi_sum,0,mpi_comm_world,ierr)
    if (rank == 0) write(*,*) 'DM Sum Global=',sum_dm 
    ...
    !$omp& private(kr,kgz,kx,ky,kz,k1,k2,w1,w2,p1,p2,p3,p4)
    do it=1,nt
       pst(:,:,it)=0
  ...
  subroutine cicmass(k)
    ...
    do j=1,np_node_dim
       do i=1,np_node_dim
          ...
          if (i1 < 0 .or. i2 .gt. nc_node_dim+1) write(*,*) 'cicmass out of bounds',i,j,k,x,y,z
