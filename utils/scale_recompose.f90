!! compile with: mpif77 -fpp -DPPINT -DBINARY scale_recompose.f90 -o recompose.x
...
real, parameter :: mesh_scale = 4.0
! normal pp run 
! real, parameter, dimension(3) :: offset = (/11.55815,-7.380774,-4.446304/)
! with pp turned off
real, parameter, dimension(3) :: offset = (/11.04736,-6.083701,5.750373/)
...
!!! THIS IS THE SCALE + OFFSET PART
   do i=1,np_local
     xv(:3,i)=xv(:3,i)-offset
     xv(:,i)=mesh_scale*xv(:,i)
   enddo
!!!
...
!! local root process writes first
    ...
    do i=1,np_local
      do j=1,3
        if (xv(j,i)<0.0 .or. xv(j,i)>=real(ncc)) then
          print *,'particle out on node 0',xv(j,i)
        endif
      enddo
    enddo
