! dm_slice_proj.f90 - serially samples a distributed checkpoint file and produces a 
! particle projection from a thin slice of the original volume
! ifort -fpp -DBINARY -DPPINT dm_slice_proj.f90 -o dmsp.x
implicit none
...
!currently only slices in z-dimension
   ...
   real(kind=4), parameter :: z_lb = 0.0  !lower bound of slice
   real(kind=4), parameter :: z_ub = 72.0  !upper bound of slice
...
!! create a slab for each checkpoint
  ...
  do cp=1,num_checkpoints
   ...
   ifile='/scratch/merz/'//z_s(1:len_trim(z_s))//'dm_slice.dat'
!! open output_slice
   open(unit=12,file=ifile,form='binary')
